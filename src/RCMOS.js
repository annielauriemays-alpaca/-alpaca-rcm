...
const TABS = ["Claim QA", "Auth Tracker", "Payer Rules", "Recovery Queue", "Automation"];
...
const AUTOMATIONS = [
  ...
  { id: "AUTO-004", name: "Eligibility hard gate", trigger: "Claim created in Candid", what: "Configure Candid's eligibility check to treat a failed or unverified eligibility result as a submission block rather than a warning. Change from warning to hard gate.", system: "Candid Health — Eligibility + Rules Engine", type: "Configure in Candid", complexity: "Low", denial_codes: ["PI-16", "membership-eligibility"], revenue_impact: "$3,649 in current membership-eligibility denials", roi: "High", phase: 1 },
  { id: "AUTO-005", name: "Denial tag routing workflow", trigger: "Claim tagged with membership-eligibility, PI-15, PR-170, or CO-B11 in Candid", what: "Configure Candid workflow rules to automatically route denial-tagged claims to the correct owner queue.", system: "Candid Health — Workflow Automation", type: "Configure in Candid", complexity: "Low", denial_codes: ["All"], revenue_impact: "Reduces denial resolution time — faster routing, fewer TFL misses. High-value denials flagged for same-day action.", roi: "Medium", phase: 1 },
      ...
      {tab === 4 && (
        <div>
          ...
          <div style={{ background: 'var(--color-background-secondary)', borderRadius: 8, padding: '10px 14px', marginBottom: '1.25rem', fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
            Configure Candid first — phases 1 items are all rules engine or workflow configuration tasks, not engineering builds. Buy or build only what Candid genuinely cannot do.
          </div>
          {[1, 2, 3].map(phase => {
